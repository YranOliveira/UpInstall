...
validate_ports() {
  ...
  printf "${WHITE} 🔄 Verificando portas em uso...${GRAY_LIGHT}"
  printf "\n\n"
  
  for port in "${ports[@]}"; do
    if [ "$(check_used_ports $port)" == "true" ]; then
      used_ports+=($port)
    fi
  done
  
  if [ ${#used_ports[@]} -gt 0 ]; then
    printf "${RED} ❌ As seguintes portas já estão em uso: ${used_ports[*]}${NC}"
    printf "\n\n"
    exit 1
  fi
  
  printf "${GREEN} ✅ Todas as portas estão disponíveis!${NC}"
  printf "\n\n"
}

# Função para verificar pasta deploy
check_deploy_folder() {
  local company_path="/home/deploy/empresas/$instancia_add"
  
  printf "${WHITE} 🔄 Verificando pasta da empresa...${GRAY_LIGHT}"
  printf "\n\n"
  
  if [ -d "$company_path" ]; then
    printf "${RED} ❌ Uma empresa com o nome '$instancia_add' já existe!${NC}"
    printf "\n\n"
    exit 1
  fi
  
  printf "${GREEN} ✅ Nome da empresa disponível!${NC}"
  printf "\n\n"
}

# Função para verificar requisitos do sistema
check_system_requirements() {
  printf "${WHITE} 🔄 Verificando requisitos do sistema...${GRAY_LIGHT}"
  printf "\n\n"
  
  # Verificar memória RAM (mínimo 4GB)
  local total_ram=$(free -g | awk '/^Mem:/{print $2}')
  if [ $total_ram -lt 4 ]; then
    printf "${RED} ❌ Seu sistema precisa ter no mínimo 4GB de RAM. Detectado: ${total_ram}GB${NC}"
    printf "\n\n"
    exit 1
  fi
  ...
  if [ $free_space -lt 20 ]; then
    printf "${RED} ❌ Seu sistema precisa ter no mínimo 20GB de espaço livre. Detectado: ${free_space}GB${NC}"
    printf "\n\n"
    exit 1
  fi
  
  printf "${GREEN} ✅ Requisitos do sistema atendidos!${NC}"
  printf "\n\n"
}

# Função para fazer backup antes de alterações
backup_before_changes() {
  local action=$1
  local timestamp=$(date +%Y%m%d_%H%M%S)
  local backup_dir="/root/whaticket_backups/${action}_${timestamp}"
  
  printf "${WHITE} 🔄 Criando backup de segurança...${GRAY_LIGHT}"
  printf "\n\n"
  
  mkdir -p $backup_dir
  
  if [ -d "/home/deploy" ]; then
    tar -czf "${backup_dir}/deploy_backup.tar.gz" /home/deploy
  fi
  
  if [ -d "/etc/nginx/sites-enabled" ]; then
    tar -czf "${backup_dir}/nginx_backup.tar.gz" /etc/nginx/sites-enabled
  fi
  
  printf "${GREEN} ✅ Backup criado em: ${backup_dir}${NC}"
  printf "\n\n"
}
